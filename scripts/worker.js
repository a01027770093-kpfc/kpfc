...
// 레거시 호환 (기존 코드에서 CORS_HEADERS 직접 참조하는 곳용)
const CORS_HEADERS = {
...
// Rate Limit 체크 (Cache API 기반 — KV 불필요)
async function checkRateLimit(ip, env) {
  ...
  count++;
  // 캐시에 카운트 저장 (TTL = RATE_LIMIT_WINDOW)
  const newResp = new Response(JSON.stringify({ count }), {
    headers: {
...
// 공격 감지 텔레그램 알림
async function sendSecurityAlert(ip, count, env) {
...
function getDateRange(period) {
  ...
  switch (period) {
    case "weekly":
      ...
      prevEndDate = formatDateKST(
        new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000),
      );
      prevStartDate = formatDateKST(
    ...
    case "monthly":
      ...
      prevStartDate = formatDateKST(
        new Date(today.getTime() - 59 * 24 * 60 * 60 * 1000),
      );
      break;
    default:
      endDate = formatDateKST(today);
...
async function getTrend(accessToken, propertyId, period) {
  ...
  const report = await runReport(accessToken, propertyId, {
    dateRanges: [
      { startDate: formatDateKST(startDate), endDate: formatDateKST(today) },
    ],
...
// handleSubmitWithData: 보안 검증 통과 후 호출 (data 이미 파싱됨)
async function handleSubmitWithData(data, env) {
  console.log("📥 KPFC 문의 접수 (보안 검증 통과)");
  const results = {
    success: true,
  ...
  if (env.AIRTABLE_TOKEN && env.AIRTABLE_BASE_ID) {
    try {
      ...
      if (rawFields["자금종류"]) {
        ...
        fields["자금종류"] = Array.isArray(fundTypes)
          ? fundTypes.join(", ")
          : fundTypes;
      }
      ...
      const customerEmailResponse = await fetch(
        "https://api.resend.com/emails",
        {
          ...
          body: JSON.stringify({
            from:
              data.emailFrom ||
              "한국정책자금지원센터 <noreply@mail.policy-fund.online>",
            to: [data.customerEmail],
            subject:
              data.customerSubject ||
              "[한국정책자금지원센터] 무료진단 신청이 접수되었습니다",
    ...
    try {
      ...
      const staffEmailResponse = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          subject:
            data.staffSubject || "[한국정책자금지원센터] 신규 무료진단 접수",
          html: data.staffHtml,
        }),
      });
...
function buildTelegramMessage(fields, submitDate, submitTime) {
  ...
  if (fields["필요자금규모"])
    msg += "├ 필요규모: " + escapeHtml(fields["필요자금규모"]) + "\n";
  if (fields["업종"]) msg += "├ 업종: " + escapeHtml(fields["업종"]) + "\n";
  if (fields["설립연도"])
    msg += "├ 설립연도: " + escapeHtml(fields["설립연도"]) + "\n";
  if (fields["직전년도매출"])
    msg += "└ 매출: " + escapeHtml(fields["직전년도매출"]) + "\n";
...
async function handleBoardAPI(request, env, path) {
  ...
  if (method === "GET" && (path === "/board" || path === "/posts")) {
    try {
      ...
      if (!airtableResponse.ok) {
        return new Response(
          JSON.stringify({ posts: [], message: "No board table or empty" }),
          {
            headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
          },
        );
...
async function handleEmployeesAPI(request, env, path) {
    ...
    try {
      ...
      if (!airtableResponse.ok) {
        return new Response(
          JSON.stringify({
            employees: [],
            message: "No employees table or empty",
          }),
          {
            headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
          },
  ...
  if (method === "PATCH" && path.startsWith("/employees/")) {
    ...
    try {
      ...
      if (data.이미지위치 !== undefined)
        fields["imagePosition"] = data.이미지위치;
...
async function handlePopupsAPI(request, env, path) {
  ...
  if (method === "GET" && path === "/popups") {
    try {
      ...
      if (!airtableResponse.ok) {
        return new Response(
          JSON.stringify({ popups: [], message: "No popups table or empty" }),
          {
            headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
          },
        );
  ...
  if (method === "PATCH" && patchMatch) {
    ...
    try {
      ...
      if (data.startDate !== undefined)
        fields["startDate"] = data.startDate || null;
...
async function handlePagesAPI(request, env, path) {
  ...
  if (!env.GITHUB_TOKEN || !env.GITHUB_OWNER || !env.GITHUB_REPO) {
    return new Response(
      JSON.stringify({
        success: false,
        error:
          "GitHub credentials not configured. Required: GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO",
      }),
      {
        status: 500,
        headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
  ...
  if (method === "POST" && updateMatch) {
    ...
    try {
      ...
      console.log(
        `✅ Page updated: ${page.path}, commit: ${result.commit.sha}`,
      );
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/upload" && request.method === "POST") {
        try {
          ...
          if (!r2Response.ok) {
            ...
            throw new Error(
              `R2 upload failed: ${r2Response.status} - ${errorText}`,
            );
          }
      ...
      // ================================================
      // 문의 접수 API (POST / 또는 /submit) — 보안 레이어 적용
      // ================================================
      if (request.method === "POST" && (path === "/" || path === "/submit")) {
        // 1) CORS Origin 검증 (폼 제출은 허용된 도메인에서만)
        const origin = request.headers.get("Origin") || "";
        ...
        // 4) 허니팟 필드 체크 (봇 트랩 — 채워져 있으면 봇)
        if (data._hp) {
          // 봇에게는 성공인 척 응답 (실제 처리 안함)
          return new Response(
            JSON.stringify({
              success: true,
              airtable: { success: true, id: "fake" },
            }),
            {
              status: 200,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        if (!rateResult.allowed) {
          // 공격 감지 알림
          await sendSecurityAlert(clientIP, rateResult.count, env);
          return new Response(
            JSON.stringify({
              success: false,
              error: "접수 횟수가 초과되었습니다. 잠시 후 다시 시도해주세요.",
            }),
            {
              status: 429,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (path === "/backfill") {
        try {
          ...
          console.log(
            `🎉 Backfill completed: ${results.length} days processed`,
          );
